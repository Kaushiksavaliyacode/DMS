  ...
  const summary = useMemo(() => {
      return data.reduce((acc, curr) => {
          ...
          if (curr.paymentType === 'cash') {
              acc.received += curr.grandTotal;
          }
          if (curr.paymentType === 'credit' && curr.challanType !== 'jobwork') {
              acc.receivable += curr.grandTotal;
          }
          return acc;
  ...
  return (
    <div className="w-full pb-20 space-y-4 font-poppins text-slate-800">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <button onClick={() => toggleSummaryFilter('all')} className={`rounded-xl border p-3 shadow-sm flex items-center justify-between transition-all ${summaryFilter === 'all' ? 'bg-indigo-50 border-indigo-500' : 'bg-white border-indigo-100'}`}>
                <div className="text-left">
                    <p className="text-[10px] font-bold text-indigo-600 uppercase">All Transactions</p>
                    <h3 className="text-lg font-bold text-indigo-700">{Math.floor(summary.all).toLocaleString()}</h3>
                </div>
...
**Reason:** Resetting cache version to `v1`.
